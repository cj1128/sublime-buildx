...
def generate_files(settings):
  ...
  color_file = os.path.join(dir, "BuildX.sublime-color-scheme")
  generate_color_file(color_file, settings)

def generate_color_file(path, settings):
  ansi_color = settings.get("ansi_color")
  ...
  # for syntax colors
  syntax_color = settings.get("syntax_color")
  ...
  with open(path, "w") as f:
    f.write(json.dumps(colorScheme, indent=2))

def plugin_loaded():
  settings = sublime.load_settings("BuildX.sublime-settings")
  generate_files(settings)
  settings.add_on_change("buildx_settings_changed", lambda: generate_files(settings))


def plugin_unloaded():
  settings = sublime.load_settings("BuildX.sublime-settings")
  settings.clear_on_change("buildx_settings_changed")

  for view in sublime.active_window().views():
    if view.name() == target_view_name:
      view.settings().clear_on_change("detect_syntax_change")
...
def is_buildx_syntax(view):
  ...
  return syntax.startswith("Packages/BuildX/BuildX.")
...
class BuildX:
  def __init__(self):
    ...
    self.buildx_syntax = True
    self.default_color_scheme = ""
    self.regions = []
    self.ansi = ANSIProcessor()

  def detect_syntax_change(self, view):
    if self.buildx_syntax and (not is_buildx_syntax(view)):
      self.buildx_syntax = False
      ...
      view.settings().set("color_scheme", self.default_color_scheme)
    ...
    if (not self.buildx_syntax) and is_buildx_syntax(view):
      self.buildx_syntax = True
  ...
  def get_target_view(self):
    ...
    view.set_syntax_file("Packages/BuildX/BuildX.sublime-syntax")
    self.default_color_scheme = view.settings().get("color_scheme")
    settings = view.settings()
    settings.set("color_scheme", buildx_color_scheme)
    settings.add_on_change("detect_syntax_change", lambda: self.detect_syntax_change(view))
  ...
  def on_build_start(self):
    ...
    self.target_view.settings().set("translate_tabs_to_spaces", not sublime.load_settings("BuildX.sublime-settings").get("preserve_content", True))
  ...
  def pipe_text(self):
    ...
    if settings.get("process_ansi_color") == True and self.buildx_syntax:
...
class BuildXListener(sublime_plugin.EventListener):
  ...
  def on_query_context(self, view, key, *args):
    ...
    source_view.settings().set("translate_tabs_to_spaces", not sublime.load_settings("BuildX.sublime-settings").get("preserve_content", True))
